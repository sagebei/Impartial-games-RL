...
def get_state_space(num_pile=5):
    state_list = []
        ...
        for ai in a:
            for bi in b:
                for ci in c:
                    for di in d:
                        for ei in e:
                            # if (sum(ai) + sum(ci) + sum(ei)) == 0 or (sum(bi) + sum(di)) == 0:
                            #     continue
...
if __name__ == '__main__':
    get_states_policies_values_masks(num_pile=5)
    # print(len([p for p in policies if len(p) > 0]))
