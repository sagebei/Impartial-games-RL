...
game.player = -1
# print(game.get_all_legal_actions())
# print(game.get_all_legal_action_indices())
# print(game.step(7))
...
# print([child.state for child in root.children.values()])
...
for i, (action, node) in enumerate(root_childrens):
    ...
    if node.state is not None: 
        for counter in list(np.array(node.state, dtype=np.int8)):
            if counter == -1:
                child_state.append(int(sum_counter))
                sum_counter = 0
            else:
                sum_counter += counter
        child_state.append(int(sum_counter))
        ...
        print(f'{child_move}: {child_state}', end='   ')
        print(f'P:{node.prior:.2f}', end="  ")
        _, value = model.predict(node.state)
        print(f'V:{-value:.2f}({(0.5-value/2)*100:.2f}%)', end=" ")
        print(f'N: {node.visit_count}({(node.visit_count/total_visit_counts)*100:.2f}%)', end='   ')
        print(f'Q value:{-node.value():.2f}', end='   ')
        print(f'WL:{0.5-node.value()/2:.2f}')
